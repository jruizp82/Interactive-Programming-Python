...
class Sprite:
    ...
    def draw(self, canvas):
        if self.animated:
            self.image_center[0] += (self.age % 24) * self.image_size[0]
        canvas.draw_image(self.image, self.image_center, self.image_size,
                          self.pos, self.image_size, self.angle)
    ...
    def collide(self, other_sprite):
        d = dist(self.pos, other_sprite.get_position())
...
# helper function: take a set and a canvas and call the 
# update and draw methods for each sprite in the group
...
# helper function:  take a set group and an a sprite other_object 
# and check for collisions between other_object and elements of the group
def group_collide(group, other_object):
    collisions = 0
    for each_sprite in set(group):
        if each_sprite.collide(other_object):
            a_explosion = Sprite(each_sprite.get_position(), [0, 0], 0, 0, explosion_image, explosion_info, explosion_sound)
            explosion_group.add(a_explosion)
            collisions += 1
            group.remove(each_sprite)
    return collisions                 
     
#  helper function group_group_collide that takes two groups of objects as input
def group_group_collide(missile_group, rock_group):
    score = 0 
    for sprite in set(missile_group):
        if group_collide(rock_group, sprite):
            score += 1
...
def rock_spawner():
    ...
    if started:
        if len(rock_group) < 12:
            ...
            if score > 100:
                rock_vel[0] += 5*rock_vel[0]
                rock_vel[1] += 5*rock_vel[1]
            elif score > 200:
                rock_vel[0] += 10*rock_vel[0]
                rock_vel[1] += 10*rock_vel[1]
            elif score > 300:
                rock_vel[0] += 20*rock_vel[0]
                rock_vel[1] += 20*rock_vel[1]
            elif score > 500:
                rock_vel[0] += 50*rock_vel[0]
                rock_vel[1] += 50*rock_vel[1]
            rock_avel = random.random() * .2 - .1
            a_rock = Sprite(rock_pos, rock_vel, 0, rock_avel, asteroid_image, asteroid_info)
            if dist(rock_pos, my_ship.pos) > 3 * my_ship.radius:
