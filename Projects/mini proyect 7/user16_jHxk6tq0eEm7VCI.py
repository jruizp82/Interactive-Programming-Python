...
class Ship:
    ...
    def shoot(self):
        global a_missile
        forward_m = angle_to_vector(self.angle)
        pos_x = self.pos[0] + forward_m[0] * self.radius
        pos_y = self.pos[1] + forward_m[1] * self.radius
        vel_x = 10 * forward_m[0] + self.vel[0]
        vel_y = 10 * forward_m[1] + self.vel[1]
...
def rock_spawner():
    ...
    ang_vel = random.choice([-1,1])*random.randrange(0,10) / 100
